...
namespace VirtualGarage.Commands
{
    public class SuperGarageAddCommand : IRocketCommand
    {
        ...
        public void Execute(IRocketPlayer caller, string[] command)
        {
            ...
            UnturnedChat.Say(caller, Plugin.Inst.Translate("virtualgarage_command_sgadd_success", vehicle.asset.vehicleName, vehicle.asset.id, garage.Name), Plugin.MsgColor);
        }
        ...
        private static bool CheckResponse(UnturnedPlayer player, string[] commands, out InteractableVehicle vehicle, out BarricadeRegion vehicleRegion)
        {
            ...
            switch (responseType)
            {
                ...
                case EResponseType.SAME_NAME_AS_GARAGE:
                    UnturnedChat.Say(player, Plugin.Inst.Translate("virtualgarage_command_vehicle_name_same_as_garage"), Plugin.MsgColor);
                    return false;
